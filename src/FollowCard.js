...
class Follow extends Component{
    ...
    componentWillMount(){
        return  http.get("http://172.16.1.71:8000/show?people="+this.props.follow_id )
        .then(data => { //data数据处理
          var name = data['name'];
    ...
    unfollow=()=>{
       if(this.state.follow){ 
           http.post("http://172.16.1.71:8000/doing?follows="+this.props.follow_id+'&sure=no').then(data=>{
            if(data['message']){message.success('取消关注！')}
            this.setState({follow:false})
            }).then(response => response, error => error);
        }
        else{
            http.post("http://172.16.1.71:8000/doing?follows="+this.props.follow_id+'&sure=yes').then(data=>{
